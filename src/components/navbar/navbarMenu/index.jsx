    ...
    <>
        ...
        <Navbar.Collapse id="responsive-navbar-nav" className="justify-content-between ms-5">
          ...
          <Nav className=" me-5 mt-3">
            <Nav.Item className="border-start border-5 border-warning p-3">
              <Nav.Link>
                <Image className="profile-img " style={{width: '40px'}} src={person} alt="profile image" />
              </Nav.Link>
            </Nav.Item>
            <div className="d-flex flex-column justify-content-center text-center mt-2">
              <Nav.Item>
                {name && <Link to="/profile/recipes" className="name-profile text-center text-decoration-none text-black">{name}</Link>}
              </Nav.Item>
              <Nav.Item>
                <Nav.Link>
              {name && <button className="logout fw-bold text-black border-0 bg-white" onClick={handleOpen}>Logout</button>}
                </Nav.Link>
              </Nav.Item>
            </div>
          </Nav>
      ...
      <Modal className="text-center d-flex justify-content-center align-items-center" show={show} onHide={handleClose} animation={true}>
        ...
        <div className="p-3 d-flex justify-content-evenly w-100">
          ...
          <Link className="w-50 ms-3" to="/login">
            <Button className="btn-warning w-100" variant="secondary" onClick={logout}>Yes</Button>
          </Link>
        </div>
      </Modal>
    </>
